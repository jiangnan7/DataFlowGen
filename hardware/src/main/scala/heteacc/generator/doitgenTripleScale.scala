...
class doitgenTripleScaleDF(implicit p: Parameters) extends doitgenTripleScaleDFIO()(p) {
  ...
  //%5 = dataflow.addr %arg1[%arg6] {memShape = [512]} : memref<512xi32>[index] -> i32
  ...
  //%7 = arith.muli %arg7, %c512 ; %8 = arith.addi %arg6, %7 : index
  val fused_gep_idx = Module(new Chain(ID = 6, NumOps = 2, OpCodes = Array("Mul", "Add"))(sign = false)(p))
  //%9 = dataflow.addr %arg3[%8] {memShape = [262656]} : memref<262656xi32>[index] -> i32
  ...
  //%12 = arith.addi ... ; %15 = arith.addi ... : i32
  ...
  //%1 = dataflow.addr %arg0[%arg8] {memShape = [512]} : memref<512xi32>[index] -> i32
  ...
  //%3 = arith.cmpi eq, %2, %c512_0 {Exe = "Loop"} : index
  ...
  val loop_0 = Module(new LoopBlockNodeExperimental(NumIns = List(1, 1, 1), NumOuts = List(1), NumCarry = List(1), NumExits = 1, ID = 0, LoopCounterMax = 512, LoopCounterStep = 1))
