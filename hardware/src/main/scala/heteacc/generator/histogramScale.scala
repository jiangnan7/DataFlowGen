...
abstract class histogramScaleDFIO(implicit val p: Parameters) extends Module with HasAccelParams {
...
class histogramScaleDF(implicit p: Parameters) extends histogramScaleDFIO()(p) {

  val FineGrainedArgCall = Module(new SplitCallDCR(argTypes = List(1, 1, 1)))
  FineGrainedArgCall.io.In <> io.in

  val mem_ctrl_cache = Module(new MemoryEngine(Size = 2048, ID = 0, NumRead = 2, NumWrite = 0))
  mem_ctrl_cache.initMem("dataset/histogramScale/in.txt")

  val mem_ctrl_cache_out = Module(new MemoryEngine(Size = 1024, ID = 0, NumRead = 1, NumWrite = 1))
  mem_ctrl_cache_out.initMem("dataset/histogramScale/out.txt")
  ...
  //%0 = dataflow.addr %arg1[%arg3] {memShape = [1024]} : memref<1024xi32>[index] -> i32
  ...
  //%2 = dataflow.addr %arg0[%arg3] {memShape = [2048]} : memref<2048xi32>[index] -> i32
  ...
  //%5 = dataflow.addr %arg2[%4] {memShape = [1024]} : memref<1024xi32>[index] -> i32
  ...
  //%8 = dataflow.addr %arg2[%4] {memShape = [1024]} : memref<1024xi32>[index] -> i32
  ...
  val loop_0 = Module(new LoopBlockNodeExperimental(NumIns = List(1, 1, 2), NumOuts = List(), NumCarry = List(3), NumExits = 1, ID = 0, LoopCounterMax = 1024, LoopCounterStep = 1))
