...
abstract class aeloss_pullDFIO(implicit val p: Parameters) extends Module with HasAccelParams {
...
class aeloss_pullDF(implicit p: Parameters) extends aeloss_pullDFIO()(p){
  ...
  //%c2_i32 = arith.constant 2 : i32
  val int_const_3 = Module(new ConstFastNode(value = 2, ID = 3))
  ...
  //%4 = dataflow.merge %c0_i32 or %arg5 {Select = "Loop_Signal"} : i32
  ...
  //%6 = dataflow.addr %arg0[%5] {memShape = [1024]} : memref<1024xi32>[index] -> i32
  ...
  //%8 = dataflow.addr %arg1[%5] {memShape = [1024]} : memref<1024xi32>[index] -> i32
  val address_5 = Module(new GepNode(NumIns = 1, NumOuts = 1, ID = 5)(ElementSize = 32, ArraySize = List()))
  ...
  val int_mul_8 = Module(new ComputeNodeWithoutState(NumOuts = 3, ID = 8, opCode = "Mul")(sign = false, Debug = false))

  //%12 = dataflow.addr %arg2[%5] {memShape = [1024]} : memref<1024xi32>[index] -> i32
  val address_9 = Module(new GepNode(NumIns = 1, NumOuts = 1, ID = 9)(ElementSize = 32, ArraySize = List()))

  //dataflow.store %11 %12 : i32 i32
  val store_10 = Module(new Store(NumOuts = 1, ID = 10, RouteID = 3))

  //%13 = arith.subi %7, %11 : i32
  val int_sub_11 = Module(new ComputeNodeWithoutState(NumOuts = 2, ID = 11, opCode = "Sub")(sign = false, Debug = false))

  //%14 = arith.subi %9, %11 : i32
  val int_sub_12 = Module(new ComputeNodeWithoutState(NumOuts = 2, ID = 12, opCode = "Sub")(sign = false, Debug = false))

  //%15 = arith.muli %13, %13 : i32
  ...
  val int_mul_14 = Module(new ComputeNodeWithoutState(NumOuts = 1, ID = 14, opCode = "Mul")(sign = false, Debug = false))
  ...
  val address_17 = Module(new GepNode(NumIns = 1, NumOuts = 1, ID = 17)(ElementSize = 32, ArraySize = List()))

  //%20 = dataflow.load %19 : i32 -> i32
  val load_18 = Module(new Load(NumOuts = 1, ID = 18, RouteID = 2))

  //%21 = arith.trunci %20 : i32 to i1
  val trunc_19 = Module(new BitCastNodeWithoutState(NumOuts = 1, ID = 19))

  //%22 = arith.addi %4, %18 : i32
  val int_add_20 = Module(new ComputeNodeWithoutState(NumOuts = 1, ID = 20, opCode = "Add")(sign = false, Debug = false))

  //%23 = dataflow.select %21, %22, %4 : i32
  val select_21 = Module(new SelectNode(NumOuts = 2, ID = 21))

  //%24 = arith.addi %5, %c1 {Exe = "Loop"} : index
  val int_add_22 = Module(new ComputeNodeWithoutState(NumOuts = 2, ID = 22, opCode = "Add")(sign = false, Debug = false))

  //%25 = arith.cmpi eq, %24, %c1024 {Exe = "Loop"} : index
  val int_cmp_23 = Module(new ComputeNodeWithoutState(NumOuts = 1, ID = 23, opCode = "eq")(sign = false, Debug = false))
  ...
  loop_0.io.CarryDepenIn(1) <> int_add_22.io.Out(1)//loop_0.io.CarryDepenOut.elements("field1")(0)
  ...
  int_mul_8.io.LeftIO <> int_const_2.io.Out

  int_mul_16.io.LeftIO <> int_const_3.io.Out

  int_add_22.io.LeftIO <> int_const_4.io.Out

  int_cmp_23.io.LeftIO <> int_const_5.io.Out

  int_add_20.io.LeftIO <> merge_1.io.Out(0)
  ...
  int_add_22.io.RightIO <> loop_0.io.CarryDepenOut.elements("field1")(4)//merge_2.io.Out(4)

  load_4.GepAddr <> address_3.io.Out(0)

  int_add_7.io.LeftIO <> load_4.io.Out(0)

  int_sub_11.io.LeftIO <> load_4.io.Out(1)

  load_6.GepAddr <> address_5.io.Out(0)

  int_add_7.io.RightIO <> load_6.io.Out(0)

  int_sub_12.io.LeftIO <> load_6.io.Out(1)

  int_mul_8.io.RightIO <> int_add_7.io.Out(0)

  store_10.inData <> int_mul_8.io.Out(0)

  int_sub_11.io.RightIO <> int_mul_8.io.Out(1)

  int_sub_12.io.RightIO <> int_mul_8.io.Out(2)

  store_10.GepAddr <> address_9.io.Out(0)

  int_mul_13.io.LeftIO <> int_sub_11.io.Out(0)

  int_mul_13.io.RightIO <> int_sub_11.io.Out(1)

  int_mul_14.io.LeftIO <> int_sub_12.io.Out(0)

  int_mul_14.io.RightIO <> int_sub_12.io.Out(1)

  int_add_15.io.LeftIO <> int_mul_13.io.Out(0)

  int_add_15.io.RightIO <> int_mul_14.io.Out(0)

  int_mul_16.io.RightIO <> int_add_15.io.Out(0)

  int_add_20.io.RightIO <> int_mul_16.io.Out(0)
  ...
  select_21.io.InData1 <> int_add_20.io.Out(0)
  ...
  int_add_7.io.enable <> exe_block_0.io.Out(12)

  int_mul_8.io.enable <> exe_block_0.io.Out(13)

  address_9.io.enable <> exe_block_0.io.Out(14)

  store_10.io.enable <> exe_block_0.io.Out(15)

  int_sub_11.io.enable <> exe_block_0.io.Out(16)

  int_sub_12.io.enable <> exe_block_0.io.Out(17)

  int_mul_13.io.enable <> exe_block_0.io.Out(18)

  int_mul_14.io.enable <> exe_block_0.io.Out(19)

  int_add_15.io.enable <> exe_block_0.io.Out(20)

  int_mul_16.io.enable <> exe_block_0.io.Out(21)
  ...
  int_add_20.io.enable <> exe_block_0.io.Out(25)
