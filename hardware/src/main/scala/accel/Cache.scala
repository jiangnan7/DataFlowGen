...
class CacheCPUIO(implicit p: Parameters) extends HeteaccGenericParameterizedBundle(p) {
...
class CacheIO(implicit p: Parameters) extends HeteaccParameterizedBundle()(p) {
  val abort = Input(Bool())
  val req = Flipped(Decoupled(new MemReq))
  val resp = Output(Valid(new MemResp))
}
...
trait CacheParams extends HasAccelParams with HasNastiParameters {
  val nWays = p(HeteaccConfigKey).nways // Not used...
  val nSets = p(HeteaccConfigKey).nsets
  val bBytes = p(HeteaccConfigKey).cacheBlockBytes
  val bBits = bBytes << 3
...
class MetaData(implicit val p: Parameters) extends HeteaccParameterizedBundle()(p) with CacheParams {
