...
class AccelTesterLocal[T <: AccelIO](c: T)
  ...
  def MemRead(addr: Int): BigInt = {
    ...
    println(s"Read $result from address $addr")
    result
  }
  ...
  def initMemory() = {
    ...
    for (i <- 0 until inAddrVec.length) {
      println(s"Writing ${inDataVec(i)} to address ${inAddrVec(i)}") // 添加打印信息调试
      MemWrite(inAddrVec(i), inDataVec(i))
      step(10)
    }
    // for (i <- inAddrVec.length / 2 until inAddrVec.length) {
    //   println(s"Writing ${inDataVec(i)} to address ${inAddrVec(i)}") // 添加打印信息调试
    //   MemWrite(inAddrVec(i), inDataVec(i))
    //   step(1)
    // }
