...
abstract class RController(NumOps: Int, BaseSize: Int, NumEntries: Int)(implicit val p: Parameters)
  ...
  val io = IO(new Bundle {
    ...
    val ReadOut = Vec(NumOps, Output(Valid(new ReadResp())))
    val MemReq = Decoupled(new MemReq)
    val MemResp = Flipped(Valid(new MemResp))
  })
}
...
class ReadMemController (NumOps: Int, BaseSize: Int, NumEntries: Int, Serialize: Boolean = true) (implicit p: Parameters)
