...
class dropout_main(implicit p: Parameters) extends AccelIO(List(32,32), List(32))(p) {
  ...
  val test13 = Module(new dropout_goldDF())//if_loop_1DF_unroll
...
class dropout_mtest01[T <: AccelIO](c: T)
  ...
  poke(c.io.in.bits.data("field1").data, 1025.U)
  ...
  poke(c.io.in.bits.data("field1").data, 1025) // Array b[] base address
  ...
  while (time < 100000 && !result) {
    ...
    println(Console.RED + s"*** Incorrect result received.  Got $data. Hoping for 522880" + Console.RESET) 
    // if(data != 0){
...
class dropout_test extends FlatSpec with Matchers {
...
120,121,122,123,124,125,126,127)
  ...
  it should s"Test: direct connection" in {
    chisel3.iotesters.Driver.execute(
      ...
      () => new dropout_main()(p)) {
      c => new dropout_mtest01(c)(inAddrVec, inDataVec, outAddrVec, outDataVec)
    } should be(true)
  }
}
