...
class histogramScale_main(implicit p: Parameters) extends AccelIO(List(32, 32, 32), List())(p) {
  ...
  val test13 = Module(new histogramScaleDF())
...
class histogramScaleDF01[T <: AccelIO](c: T)
  ...
  while (time < 50000 && !result) {
    ...
    if (peek(c.io.out.valid) == 1) {
      result = true
      println(Console.BLUE + s"*** histogramScale finished. Run time: $time cycles." + Console.RESET)
    }
  }
...
class histogramScaleDF_test extends FlatSpec with Matchers {

  val inDataVec = List()
  val inAddrVec = List.range(0, 32 * inDataVec.length, 32)
  val outAddrVec = List.range(32 * inDataVec.length, 32 * inDataVec.length + (32 * 1024), 32)
  val outDataVec = List()
  ...
  it should s"Test: direct connection" in {
    Driver.execute(
      Array(
        "-tn", "histogramScale",
        "-tbn", "verilator",
        "-td", s"test_run_dir/histogramScale",
        "-tts", "0001",
        "--generate-vcd-output", "on"),
      () => new histogramScale_main()(p)) {
      c => new histogramScaleDF01(c)(inAddrVec, inDataVec, outAddrVec, outDataVec)
    } should be(true)
  }
}
