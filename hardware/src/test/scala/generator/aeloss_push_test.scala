...
// 测试模块
class aeloss_push_main(implicit p: Parameters) extends AccelIO(List(32, 32), List(32))(p) {
  ...
  val test13 = Module(new aeloss_pushDF())//if_loop_1DF_unroll
...
class aeloss_push_test01[T <: AccelIO](c: T)
  ...
  var result = false
  while (time < 99999999 && !result) {
    time += 1
    step(1)
    val data = peek(c.io.out.bits.data("field0").data)
    val dtat = peek(c.io.in.bits.data("field0").data)
    println(Console.RED + s"*** Incorrect result received. input $dtat Got $data. Hoping for 161700" + Console.RESET) 
    // if(data != 0){
...
class aeloss_push_test extends FlatSpec with Matchers {
  ...
  it should s"Test: direct connection" in {
    chisel3.iotesters.Driver.execute(
      Array(
        ...
        "-tn", "aeloss_push",
        "-tbn", "verilator",
        "-td", s"test_run_dir/aeloss_push",
        "-tts", "0001",
        "--generate-vcd-output", "on"),
        
      () => new aeloss_push_main()(p)) {
      c => new aeloss_push_test01(c)(inAddrVec, inDataVec, outAddrVec, outDataVec)
    } should be(true)
  }
}
