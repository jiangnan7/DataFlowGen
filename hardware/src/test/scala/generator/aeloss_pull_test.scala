...
// 测试模块
class aeloss_pull_main(implicit p: Parameters) extends AccelIO(List(32, 32,32 ,32), List(32))(p) {
  ...
  val test13 = Module(new aeloss_pullDF())//if_loop_1DF_unroll
...
class aeloss_pull_test01[T <: AccelIO](c: T)
...
class aeloss_pull_test extends FlatSpec with Matchers {
  ...
  it should s"Test: direct connection" in {
    chisel3.iotesters.Driver.execute(
      Array(
        ...
        "-tn", "aeloss_pull",
        "-tbn", "verilator",
        "-td", s"test_run_dir/aeloss_pull",
        "-tts", "0001",
        "--generate-vcd-output", "on"),
        
      () => new aeloss_pull_main()(p)) {
      c => new aeloss_pull_test01(c)(inAddrVec, inDataVec, outAddrVec, outDataVec)
    } should be(true)
  }
}
