...
class matrix_add01[T <: AccelIO](c: T)
  ...
  while (time < 40000 && !result) {
    ...
    println(Console.RED + s"*** Incorrect result received.  Got $data. Hoping for 20200" + Console.RESET) 
    // if(data != 0){
...
class matrix_add_test extends FlatSpec with Matchers {
