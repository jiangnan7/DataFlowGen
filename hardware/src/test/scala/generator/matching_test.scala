...
// 测试模块
class matching_main(implicit p: Parameters) extends AccelIO(List(32, 32), List())(p) {
...
class matchingDF01[T <: AccelIO](c: T)
  ...
  poke(c.io.in.bits.data("field1").data, 1) // Array b[] base address
...
class matching_test extends FlatSpec with Matchers {
  ...
  it should s"Test: direct connection" in {
    chisel3.iotesters.Driver.execute(
      Array(
        ...
        "-tn", "matching",
        "-tbn", "verilator",
        "-td", s"test_run_dir/matching",
        "-tts", "0001",
        "--generate-vcd-output", "on"),
        
      () => new matching_main()(p)) {
      c => new matchingDF01(c)(inAddrVec, inDataVec, outAddrVec, outDataVec)
    } should be(true)
  }
}
