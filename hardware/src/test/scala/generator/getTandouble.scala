...
class getTanhDoubleDF01[T <: AccelIO](c: T)
  ...
  // poke(c.io.in.bits.data("field1").data, 3200) // Array b[] base address
  ...
  while (time < 50000 && !result) {
    ...
    val data = peek(c.io.out.bits.data("field0").data)
    println(Console.RED + s"*** Incorrect result received. input $data Got. Hoping for 100" + Console.RESET) 
    // if(data != 0){
