...
// 测试模块
class stencil_float_main(implicit p: Parameters) extends AccelIO(List(32, 32,32), List())(p) {
  ...
  val test13 = Module(new stencil_floatDF())//if_loop_1DF_unroll
...
class stencil_float_test01[T <: AccelIO](c: T)
...
class stencil_float_test extends FlatSpec with Matchers {
  ...
  it should s"Test: direct connection" in {
    chisel3.iotesters.Driver.execute(
      Array(
        ...
        "-tn", "stencil_float",
        "-tbn", "verilator",
        "-td", s"test_run_dir/stencil_float",
        "-tts", "0001",
        "--generate-vcd-output", "on"),
        
      () => new stencil_float_main()(p)) {
      c => new stencil_float_test01(c)(inAddrVec, inDataVec, outAddrVec, outDataVec)
    } should be(true)
  }
}
