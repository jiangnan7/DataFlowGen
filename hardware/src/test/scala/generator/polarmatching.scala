...
// 测试模块
class polarmatchingDF_main(implicit p: Parameters) extends AccelIO(List(32, 32), List())(p) {
  ...
  val test13 = Module(new polarmatchingDF())
...
class polarmatchingDFDF01[T <: AccelIO](c: T)
  ...
  poke(c.io.in.bits.data("field1").data, 2048.U)
  ...
  poke(c.io.in.bits.data("field1").data, 2048) // Array b[] base address
...
class polarmatchingDF_test extends FlatSpec with Matchers {
  ...
  it should s"Test: direct connection" in {
    chisel3.iotesters.Driver.execute(
      Array(
        ...
        "-tn", "polarmatchingDF",
        "-tbn", "verilator",
        "-td", s"test_run_dir/polarmatchingDF",
        "-tts", "0001",
        "--generate-vcd-output", "on"),
        
      () => new polarmatchingDF_main()(p)) {
      c => new polarmatchingDFDF01(c)(inAddrVec, inDataVec, outAddrVec, outDataVec)
    } should be(true)
  }
}
