...
struct HybridAnalysis : public HybridAnalysisBase<HybridAnalysis> {
  ...
  void runOnOperation() override {
    ...
    func.walk([&](dataflow::ForOp forOp) {
      ...
      if (loopBand != -1) {
        if (bandMap.count(loopBand) == 0) {
          ...
          int existingLevel =
              existingOp->getAttrOfType<mlir::IntegerAttr>("Loop_Level")
                  .getInt();
    ...
    for (auto &bandEntry : bandMap) {
      ...
      int maxUnrollFactor =
          loopInfo["upper_bound"].get<int>() / loopInfo["step"].get<int>();
      ...
      unroll_factor = {{"max", maxUnrollFactor}, {"min", minUnrollFactor}};

      nlohmann::json vector_factor;
      vector_factor = {{"max", maxVectorFactor}, {"min", minVectorFactor}};
