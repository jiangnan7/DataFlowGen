...
SmallVector<Superword *, 2> Superword::getOperands() const {
  ...
  for (auto const &operand : this->operandWords) {
...
VectorType Superword::getVectorType() const {
  return VectorType::get(static_cast<unsigned>(this->numLanes()),
                         this->getElementType());
}

Type Superword::getElementType() const { return this->getElement(0).getType(); }

Location Superword::getLoc() const { return this->getElement(0).getLoc(); }
...
SLPGraph::SLPGraph(ArrayRef<Value> seed, unsigned maxNodeSize,
  ...
  SLPGraphBuilder{*this, maxNodeSize, maxLookAhead, allowDuplicateElements}
      .build(seed);
}
