...
bool ConversionState::alreadyComputed(Value value) const {
  return permanentData.alreadyComputed(value) ||
         temporaryData.alreadyComputed(value);
}
...
void ConversionState::markComputed(Value value) {
  assert(!alreadyComputed(value) &&
         "marking already computed value as computed");
...
void ConversionState::markComputed(Superword *superword, Value value) {
  ...
  for (size_t lane = 0; lane < superword->numLanes(); ++lane) {
    if (!superword->hasAlteredSemanticsInLane(lane)) {
      temporaryData.extractableScalarValues.try_emplace(
          superword->getElement(lane), superword, lane);
    }
  }
...
void ConversionState::markExtracted(Value value) {
  assert(!alreadyComputed(value) &&
         "extracting value that has been marked as computed already");
...
void ConversionState::addVectorCallbacks(
    std::function<void(Superword *)> createCallback,
...
void ConversionState::addScalarCallbacks(
    std::function<void(Value)> inputCallback,
...
void ConversionState::addExtractionCallbacks(
    std::function<void(Value)> extractCallback,
    std::function<void(Value)> undoCallback) {
...
ConversionManager::ConversionManager(RewriterBase &rewriter, Block *block,
                                     CostModel *costModel,
...
ConversionManager::startConversion(SLPGraph const &graph) {
...
void ConversionManager::cancelConversion() {
  ...
  for (auto *op : originalOperations) {
    // Replace extracted operands of escaping users with their original
    // operands.
    auto operands = originalOperands.lookup(op);
    if (!operands.empty()) {
      op->setOperands(operands);
    }
    // Move all operations created during conversion behind the last original
    // one.
    if (lastOp) {
      op->moveAfter(lastOp);
    } else {
...
void ConversionManager::setupConversionFor(
    Superword *superword, SLPVectorizationPattern const *pattern) {
  ...
  for (size_t lane = 0; lane < superword->numLanes(); ++lane) {
    auto element = superword->getElement(lane);
    if (std::find(std::begin(scalarInputs), std::end(scalarInputs), element) ==
        std::end(scalarInputs)) {
      superword->setElement(lane, getOrExtractValue(element));
    }
  }
}

void ConversionManager::update(Superword *superword, Value operation,
...
Value ConversionManager::getOrCreateConstant(Location const &loc,
                                             Attribute const &attribute) {

  return folder.getOrCreateConstant(
      rewriter, &attribute.getDialect(), attribute,
      attribute.dyn_cast<TypedAttr>().getType(), loc);
}
...
Value ConversionManager::getOrExtractValue(Value value) {
  ...
  auto const &wordPosition =
      conversionState->getSuperwordContainingValue(value);
  assert(
      !wordPosition.superword->hasAlteredSemanticsInLane(wordPosition.index));
  assert(
      wordPosition.superword &&
      "extraction deemed profitable, but value does not appear in any vector");
  auto source = conversionState->getValue(wordPosition.superword);
  auto pos = getOrCreateConstant(
      source.getLoc(), rewriter.getI32IntegerAttr((int)wordPosition.index));
  auto extractOp =
      rewriter.create<vector::ExtractElementOp>(value.getLoc(), source, pos);
...
// Helper functions in anonymous namespace.
namespace {
/// Reorder operations in a BFS manner by computing their depths from the block
/// entry.
void reorderOperationsBFS(Block *block) {
  ...
  // Sort each group according to the current block order (prevents placing uses
  // in front of defs).
  for (auto &ops : opsSortedByDepth) {
    llvm::sort(std::begin(ops), std::end(ops),
               [&](Operation *lhs, Operation *rhs) {
                 return lhs->isBeforeInBlock(rhs);
               });
  }
...
void reorderOperationsDFS(Block *block) {
  ...
  for (auto it = block->rbegin(); it != block->rend(); ++it) {
    auto *op = &(*it);
    if (order.contains(op) || op->getNumOperands() == 0 ||
    ...
    while (!stack.empty()) {
      ...
      // Reverse order due to LIFO stack. Otherwise, RHS operands would appear
      // before LHS operands in the final code.
  ...
  // Rearrange the block's operations according to the DFS order we just
  // determined.
  Operation *lastOp = nullptr;
  for (auto *op : order) {
    if (lastOp) {
