...
ArgumentNode *
ContainerNode::insertLiveInArgument(Value val,
  ...
  case ContainerNode::ContainType::LoopNodeTy: {
    ...
    if (ff == this->live_in_sets_end()) {
      this->live_in_sets.push_back(std::make_unique<ArgumentNode>(
          NodeInfo(this->live_in_sets.size(),
                   "live_in_" + std::to_string(this->live_in_sets.size())),
          type, valType, val, this));

      ff = std::find_if(this->live_in_sets_begin(), this->live_in_sets_end(),
                        [&val](auto &arg) -> bool {
                          return arg.get()->getArgumentValue() == val;
                        });
    }
  ...
  case ContainerNode::ContainType::FuncCallTy: {
    if (llvm::isa<mlir::MemRefType>(val.getType())) {
      ...
      if (find == this->live_in_mems_end()) {
        this->live_in_mems.push_back(std::make_unique<ArgumentNode>(
            NodeInfo(this->live_in_mems.size(),
                     "arg_mem" + std::to_string(this->live_in_mems.size())),
            type, DataType::MemrefType, val, this));
      }
      find = std::find_if(this->live_in_mems_begin(), this->live_in_mems_end(),
                          [&val](auto &arg) -> bool {
                            return arg.get()->getArgumentValue() == val;
                          });
      return find->get();
      ...
      if (ff == this->live_in_vals_end()) {
        ...
        this->live_in_vals.push_back(std::make_unique<ArgumentNode>(
            NodeInfo(this->live_in_vals.size(), "arg_val"), type, tempType, val,
            this));
        ff = std::find_if(this->live_in_vals_begin(), this->live_in_vals_end(),
                          [&val](auto &arg) -> bool {
                            return arg.get()->getArgumentValue() == val;
                          });
      }
...
ContainerNode::insertLiveOutArgument(Value val,
  ...
  if (ff == this->live_out_sets.end()) {
    this->live_out_sets.push_back(std::make_unique<ArgumentNode>(
        NodeInfo(this->live_out_sets.size(),
                 "live_in_" + std::to_string(this->live_out_sets.size())),
        type, valType, val, this));

    ff = std::find_if(this->live_out_sets.begin(), this->live_out_sets.end(),
                      [&val](auto &arg) -> bool {
                        return arg.get()->getArgumentValue() == val;
                      });
  }
...
ContainerNode::insertCarryDepenArgument(Value val,
  ...
  if (ff == this->carry_depen_sets.end()) {
    this->carry_depen_sets.push_back(std::make_unique<ArgumentNode>(
        NodeInfo(this->carry_depen_sets.size(),
                 "carry_" + std::to_string(this->carry_depen_sets.size())),
        argtype, valType, val, this));

    ff = std::find_if(this->carry_depen_sets.begin(),
                      this->carry_depen_sets.end(), [&val](auto &arg) -> bool {
                        return arg.get()->getArgumentValue() == val;
                      });
  }
...
Node *ContainerNode::findCarryDepenNode(Value val) {
  auto ff =
      std::find_if(this->carry_depen_sets.begin(), this->carry_depen_sets.end(),
                   [&val](auto &arg) -> bool {
                     return arg.get()->getArgumentValue() == val;
...
uint32_t ContainerNode::findLiveInArgumentIndex(ArgumentNode *_arg_node) {
  ...
  case ContainType::FuncCallTy: {
    if (_arg_data_type == DataType::MemrefType) {
      std::copy_if(live_in_mems.begin(), live_in_mems.end(),
                   std::back_inserter(_local_list), find_function);
    } else if (_arg_data_type == DataType::IntegerType ||
               _arg_data_type == DataType::FloatType) {
...
uint32_t ContainerNode::findLiveOutArgumentIndex(ArgumentNode *_arg_node) {
  ...
  std::copy_if(live_out_sets.begin(), live_out_sets.end(),
               std::back_inserter(_local_list), find_function);
...
uint32_t ContainerNode::findCarryDepenArgumentIndex(ArgumentNode *_arg_node) {
  ...
  std::copy_if(this->carry_depen_sets.begin(), this->carry_depen_sets.end(),
               std::back_inserter(_local_list), find_function);
...
Node *ContainerNode::findLiveInNode(Value val) {
  ...
  case ContainerNode::ContainType::FuncCallTy: {
    if (llvm::isa<mlir::MemRefType>(val.getType())) {
      ...
      auto ff =
          std::find_if(this->live_in_vals.begin(), this->live_in_vals.end(),
                       [&val](auto &arg) -> bool {
                         return arg.get()->getArgumentValue() == val;
                       });
...
Node *ContainerNode::findLiveOutNode(Value val) {
  auto ff = std::find_if(live_out_sets.begin(), live_out_sets.end(),
                         [&val](auto &arg) -> bool {
                           return arg.get()->getArgumentValue() == val;
                         });
...
mlir::Operation *OperationNode::getOperation() { return this->parent_op; }
...
PortID Node::returnControlOutputPortIndex(Node *_node) {
  auto ff =
      std::find_if(this->port_control.control_output_port.begin(),
                   this->port_control.control_output_port.end(),
                   [&_node](auto &arg) -> bool { return arg.first == _node; });
...
PortID Node::addControlInputPort(Node *n) {
  ...
  this->port_control.control_input_port.emplace_back(
      std::make_pair(n, port_info));
  return port_info;
}
...
PortID Node::addControlOutputPort(Node *n) {
  ...
  this->port_control.control_output_port.emplace_back(
      std::make_pair(n, port_info));
  return port_info;
}
