...
LoopInfo GraphGen::analyzeLoopNode(dataflow::ForOp dataflowForop) {
  ...
  LoopInfo Info("loop_" +
                std::to_string(this->dependency_graph->getLoopNodeNums()));
  ...
  // FIXME
  dataflowForop->getParentOp()->getParentRegion()->front().walk(
      [&](mlir::Operation *outer) {
        if (auto state = dyn_cast<dataflow::StateOp>(outer)) {
          if (state.getInstanceName().str().find("loop_start") !=
              std::string::npos) {
            Info.enable = outer;
            outer->dump();
            blacklist_control_edge[outer].push_back(
                dataflowForop.getOperation());
  ...
  if (Info.enable == nullptr) {
    ...
    blacklist_control_edge[dataflowForop.getOperation()->getParentOp()]
        .push_back(dataflowForop.getOperation());
  }
  ...
  for (
      auto &for_exe_op :
      Info.exe_block_op->getRegion(0)
          .front()) { // dataflowForop.getRegion().front().walk([&](mlir::Operation*
                      // for_exe_op){
    if (auto state = dyn_cast<dataflow::StateOp>(for_exe_op)) {
      {
        ...
        blacklist_control_edge[&for_exe_op].push_back(
            dataflowForop.getOperation());
        // return WalkResult::interrupt();
        break;
      }
  ...
  // auto liveins =
  // Liveness(dataflowForop.getOperation()).getLiveIn(&dataflowForop.getRegion().front());
  ...
  dataflowForop.getOperation()->walk([&](mlir::Operation *region_op) {
    if (isa<dataflow::ExecutionBlockOp>(region_op) ||
        this->op2traversal.find(region_op) != this->op2traversal.end())
    ...
    for (const auto &operand : region_op->getOperands()) {
      ...
      if (is_live_in) {
        ...
        // auto iter = std::find(livein2op.begin(),
        // this->dependency_graph->funArgValue.end(), operand);
        // llvm::SmallVector<mlir::Operation*, 8> livein2op_set =
        // livein2op[operand]; if(livein2op_set) llvm::outs() << "xxx" << "\n";
          ...
          for (auto &sub_loop_op : sub_loop.getLoopBody().front()) {
            ...
            for (const auto &sub_loop_operand : sub_loop_op.getOperands()) {
              ...
              if (&operand == &sub_loop_operand) {
                loop2loop = true;
                LLVM_DEBUG(llvm::dbgs()
                               << "Find the live-in value to loop1. \n ";);

                //       mlir::Value* nonConstOperand =
                //       const_cast<mlir::Value*>(operand);
              ...
              if (isa<mlir::BlockArgument>(sub_loop_operand)) {
                LLVM_DEBUG(llvm::dbgs() << "Find the live-in value to loop2.   "
                                        << operand << "   \n";);
                Info.live_in_out_loop[operand].insert(sub_loop);
                // sub_loop_op.dump();
              }
        ...
        //     }
        //     else {
        //       dataflowForop->getParentOp()->walk([&](dataflow::ForOp
        //       parent_loop){
        //       // if(&dataflowForop == &parent_loop) return
        //       WalkResult::advance(); for(auto &parent_loop_op :
        //       parent_loop.getLoopBody().front()){
  ...
  //   if(isa<dataflow::ExecutionBlockOp, arith::ConstantOp>(region_op) ||
  //   this->op2traversal[region_op])
  //     return WalkResult::advance(); //dataflow.state
  ...
  //   //     is_live_out = !dataflowForop->isAncestor(user.getOwner()) ? true :
  //   false;
  //   //     if(is_live_out){
      ...
      for (const auto &use :
        ...
        if (dyn_cast<dataflow::TaskOp>(use.getDefiningOp()->getParentOp())) {
          ...
          for (auto &forResultUser : use.getUses()) {
            ...
            this->blacklist_loop_live_out_data_edge[forInduction].push_back(
                forResultUser.getOwner());
          }
  ...
  for (const auto &iter_value : dataflowForop.getRegionIterArgs()) {
    ...
    for (auto &use : iter_value.getUses()) {
      ...
      // }
      auto yield_op_value =
          Info.exe_block_op->getRegion(0).front().getTerminator()->getOperand(
              carry_index);

      Info.carry_dependencies[iter_value].push_back(use.getOwner());
      Info.carry_dependencies[iter_value].push_back(
          yield_op_value.getDefiningOp());
      Info.carry_dependencies_without_connection[iter_value].push_back(
          yield_op_value.getDefiningOp());

      Info.carry_dependencies_map[use.getOwner()] = iter_value;
      Info.carry_dependencies_map[yield_op_value.getDefiningOp()] = iter_value;

      this->blacklist_carry_dependency_data_edge[iter_value].push_back(
          use.getOwner());
...
void GraphGen::buildLoopGraph(func::FuncOp func) {
  ...
  func.walk([&](dataflow::ForOp forOp) {
    ...
    this->map_op_node[currLoopInfo.enable]->addControlOutputPort(
        currentLoopNode);
    currentLoopNode->setControlLoop(this->map_op_node[currLoopInfo.enable]);

    currentLoopNode->setActiveOutputLoopSignal(
    ...
    if (dyn_cast<dataflow::TaskOp>(forOp->getParentOp())) {
      if (!this->dependency_graph->hasreturnValue) {
        ...
        this->map_value_node[this->dependency_graph->returnValue]
            ->addControlInputPort(currentLoopNode);
        currentLoopNode->setActiveExitSignal(
            this->map_value_node[this->dependency_graph->returnValue]);
      }
    } else {
      ;
  ...
  func.walk([&](dataflow::ForOp forOp) {
    ...
    for (auto in : currLoopInfo.live_in_in_ins) {
      ...
      if (node == nullptr) {
        auto new_live_in = currentLoopNode->insertLiveInArgument(
            in, ArgumentNode::ArgumentType::LoopLiveIn);
    ...
    for (auto in : currLoopInfo.live_in_out_loop) {
      ...
      for (auto &loop : in.getSecond()) {
        ...
        this->live_in_loop_loop_edge[in.getFirst()].insert(
            std::make_pair(forOp, loop));
      }
    }

    for (auto src : currLoopInfo.live_in_out_ins) {
      for (auto tar : src.getSecond()) {
        this->live_in_loop_ins_edge[forOp].insert(
            std::make_pair(src.getFirst(), tar));
      }
    }
    ...
    for (auto _live_out : currLoopInfo.live_out_in_ins) {
      ...
      if (_new_live_out_node == nullptr) {
        _new_live_out_node = currentLoopNode->insertLiveOutArgument(
            _live_out, ArgumentNode::ArgumentType::LoopLiveOut);
        LLVM_DEBUG(llvm::dbgs() << "_new_live_out_node is "
                                << _new_live_out_node->getName() << "\n";);
        if (this->map_value_node.find(_live_out) ==
            this->map_value_node.end()) {
    ...
    for (auto _live_out : currLoopInfo.live_out_in_loop) {
      ...
      for (auto _n : _live_out.getSecond()) {
        auto _tmp =
            this->for_op_node[_n]->findLiveOutNode(_live_out.getFirst());
        if (_tmp == nullptr) {
          this->for_op_node[_n]->insertLiveOutArgument(
              _live_out.getFirst(), ArgumentNode::ArgumentType::LoopLiveOut);
        }
        this->loop_loop_edge_lout_map[_live_out.getFirst()].push_back(
            std::make_pair(forOp, _n));
      }
    }
    ...
    LLVM_DEBUG(llvm::dbgs() << "Edge type 3 "
                            << currLoopInfo.carry_dependencies.size() << "\n";);

    // Connecting carry values
    for (auto carry_value : currLoopInfo.carry_dependencies) {
      auto new_carry_depen = currentLoopNode->insertCarryDepenArgument(
          carry_value.getFirst(), ArgumentNode::ArgumentType::CarryDependency);
      if (this->map_value_node.find(carry_value.getFirst()) ==
          this->map_value_node.end()) {
        this->map_value_node[carry_value.getFirst()] = new_carry_depen;
      }

      new_carry_depen->setParentNode(
          this->map_value_node[carry_value.getFirst()]);

      for (auto use : carry_value.getSecond()) {

        bool find_carry_out = llvm::any_of(
            use->getBlock()->back().getOperands(),
            [&](mlir::Value value) { return use->getResult(0) == value; });
        bool find_carry_in =
            llvm::any_of(use->getOperands(), [&](mlir::Value value) {
              return carry_value.getFirst() == value;
            });

        this->loop_edge_map[std::make_pair(carry_value.getFirst(), use)] =
            new_carry_depen;
        ...
        if (find_carry_in && !find_carry_out) {
          if (auto addAttr = use->getAttrOfType<StringAttr>("Exe")) {
            ...
            if (addValue.find("Loop") != std::string::npos) {
              this->edge_direction_map[std::make_pair(
                  carry_value.getFirst(), use)] = 3; // op --> carry value
            }
          }
        }
        ...
        if (dyn_cast<dataflow::MergeOp>(use) && find_carry_in) {
          this->edge_direction_map[std::make_pair(
              carry_value.getFirst(), use)] = 1; // carry value --> op
        } else if (find_carry_out && !find_carry_in) {
          this->edge_direction_map[std::make_pair(
              carry_value.getFirst(), use)] = 2; // op --> carry value
...
void GraphGen::dependencyAnalyze(mlir::func::FuncOp func) {
  ...
  for (auto &value : this->funArgValue) {
    auto fine_arg_node =
        static_cast<ArgumentNode *>(this->map_value_node[value]);
  ...
  func.walk([&](dataflow::StateOp operation) {
    if (operation.getOperation()->hasAttr("Enable")) {
      auto en_branch = static_cast<StateBranchNode *>(
          this->map_op_node[operation.getOperation()]);
  ...
  func.walk([&](mlir::Operation *operation) {
    ...
    if (isa<arith::ConstantOp>(operation) ||
        isa<dataflow::EnableOp>(operation) || isa<dataflow::ForOp>(operation) ||
        isa<dataflow::StateOp>(operation)) {
      return WalkResult::advance();
    }
    ...
    for (uint32_t i = 0; i < operation->getNumOperands(); ++i) {
      ...
      if (isa<mlir::BlockArgument>(operand)) {
        ...
        const_node->addControlInputPort(
            this->map_op_node[operation->getParentOp()]);
        this->map_op_node[operation->getParentOp()]->addControlOutputPort(
            const_node);
        static_cast<ExecutionBlockNode *>(
            this->map_op_node[operation->getParentOp()])
      ...
      if (find_const) {
        ...
        to->second->addDataInputPort(
            const_node,
            i); // Fixed: Resolved an issue with incorrect port handling.
      }
    }
    return WalkResult::advance();
  });
  ...
  func.walk([&](mlir::Operation *operation) {
    ...
    for (uint32_t i = 0; i < operation->getNumOperands(); ++i) {
      ...
      if (find_live_in) {
        for (auto _loop_edge : this->live_in_loop_ins_edge) {
          ...
          if (_edge != _loop_edge.getSecond().end()) {
            ...
            _edge->second->dump();
            LLVM_DEBUG(llvm::dbgs() << _node_src->getName() << " "
                                    << _node_tar->getName() << "\n";);
            _node_src->addDataOutputPort(_node_tar);
            _node_tar->addDataInputPort(
                _node_src,
                i); // Fixed: Resolved an issue with incorrect port handling.
          }
        }
      ...
      for (auto &data_scr : this->blacklist_carry_dependency_data_edge) {
        ...
        // and the result of the operation is used as input to the loop
        // iteration.
        else if (!llvm::is_contained(conflict_op, operation)) {
          find_carry_out =
              llvm::any_of(operation->getBlock()->back().getOperands(),
                           [&](mlir::Value value) {
                             return operation->getResult(0) == value;
                           }) &&
      ...
      if (find_carry) {
        ...
        // Operation --> Carry
        LLVM_DEBUG(llvm::dbgs()
                       << from->second->getName() << " " << _carry->getName()
                       << " " << to->second->getName() << "\n";);
        ...
        auto curr_carry =
            this->for_op_info[dyn_cast<dataflow::ForOp>(
                                  operation->getParentOp()->getParentOp())]
                .carry_dependencies_map[operation];
        assert(
            this->edge_direction_map[std::make_pair(curr_carry, operation)] >=
            2);
        auto _carry =
            this->loop_edge_map[std::make_pair(curr_carry, operation)];
        LLVM_DEBUG(llvm::dbgs() << "find_carry_out: " << _carry->getName()
                                << " " << to->second->getName() << "\n";);
  ...
  func.walk([&](dataflow::ExecutionBlockOp exeop) {
    if (auto exeblock = static_cast<ExecutionBlockNode *>(
            this->map_op_node[exeop.getOperation()])) {
      // LLVM_DEBUG(llvm::dbgs() << "exeop.\n " ;);
      // exeop.dump();
      for (auto &inner : exeop.getBody().front()) {
        // if(isa<arith::IndexCastOp>(inner)){//dataflow::IfOp,
        // dataflow::ExecutionBlockOp,
        //   continue;
        // }
        auto iter_node = this->map_op_node.find(&inner);
        if (iter_node == map_op_node.end())
          continue;
        if (auto operation_node =
                static_cast<OperationNode *>(this->map_op_node[&inner])) {
          ...
          if (operation_node->getOperationType() ==
              OperationNode::OperationType::MergeType) {
...
void GraphGen::connectingBranch(mlir::func::FuncOp func) {
  ...
  func.walk([&](dataflow::StateOp operation) {
    if (operation.getInstanceName().str().find("if_then") !=
        std::string::npos) {
      this->state2if[operation] =
  ...
  for (auto &op : this->state2if) {
    ...
    static_cast<StateBranchNode *>(this->map_op_node[op.getFirst()])
        ->addDataInputPort(if_cmp);
    if_cmp->addDataOutputPort(
        static_cast<StateBranchNode *>(this->map_op_node[op.getFirst()]));

    if (!op.getSecond().getElseRegion().empty()) {
      auto stateNode =
          static_cast<StateBranchNode *>(this->map_op_node[op.getFirst()]);
      stateNode->addTrueBranch(
          this->map_op_node[&*op.getSecond().getThenRegion().front().begin()]);
      stateNode->addFalseBranch(
          this->map_op_node[&*op.getSecond().getElseRegion().front().begin()]);

      this->map_op_node[&*op.getSecond().getElseRegion().front().begin()]
          ->addControlInputPortIndex(stateNode, 0);
      this->map_op_node[&*op.getSecond().getThenRegion().front().begin()]
          ->addControlInputPortIndex(stateNode, 0);
    }
  }
  ...
  //   storeInblock[op] =
  //   dyn_cast<dataflow::ExecutionBlockOp>(op->getParentOp());
  // });
  ...
  for (auto &op : selectops) {
    ...
    auto if_then_exeield =
        if_then_exe.getRegion(0).front().getTerminator()->getOperand(0);
    ...
    auto if_else_exeYield =
        if_else_exe.getRegion(0).front().getTerminator()->getOperand(0);
...
void Graph::connectingGraph(mlir::func::FuncOp func) {
  ...
  for (auto &const_node : this->const_list) {
    for (auto iter = const_node->outputDataport_begin();
         iter != const_node->outputDataport_end(); iter++) {
      if (isa<ArgumentNode>(&*iter->first))
        continue;
      this->addEdge(
          Edge::EdgeType::DataTypeEdge,
          std::make_pair(&*const_node, const_node->returnDataOutputPortIndex(
                                           &(*iter->first))),
  ...
  for (auto iter = this->getArgCall()->live_in_mems_begin();
       iter != this->getArgCall()->live_in_mems_end(); iter++) {
    ...
    for (auto iter_data = iter->get()->outputDataport_begin();
      ...
      this->addEdge(
          Edge::EdgeType::DataTypeEdge,
          std::make_pair(&*iter->get(), iter->get()->returnDataOutputPortIndex(
                                            &*iter_data->first)),
          std::make_pair(
              &*iter_data->first,
  ...
  for (auto iter = this->getArgCall()->live_in_vals_begin();
       iter != this->getArgCall()->live_in_vals_end(); iter++) {
    if (iter->get()->getArgType() != ArgumentNode::ArgumentType::LiveIn)
      continue;
