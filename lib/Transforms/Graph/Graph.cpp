...
CmpNode *Graph::insertCmpNode(Value result, DataType type) {
  ...
  this->op_list.push_back(std::make_unique<CmpNode>(
      NodeInfo(op_list.size(), name), OperationNode::OperationType::CmpType,
      type, result.getDefiningOp(), code

      ));

  auto ff = std::find_if(
      op_list.begin(), op_list.end(), [&result](auto &arg) -> bool {
...
ComputeOperationNode *Graph::insertShrUINode(Value result, DataType type) {
  ...
  this->op_list.push_back(std::make_unique<ComputeOperationNode>(
      NodeInfo(op_list.size(), name), OperationNode::OperationType::BinaryType,
      type, result.getDefiningOp(), OpCode::lshr
...
ComputeOperationNode *Graph::insertShrNode(Value result, DataType type) {
  ...
  this->op_list.push_back(std::make_unique<ComputeOperationNode>(
      NodeInfo(op_list.size(), name), OperationNode::OperationType::BinaryType,
      type, result.getDefiningOp(), OpCode::shr
...
ComputeOperationNode *Graph::insertShlNode(Value result, DataType type) {
  ...
  this->op_list.push_back(std::make_unique<ComputeOperationNode>(
      NodeInfo(op_list.size(), name), OperationNode::OperationType::BinaryType,
      type, result.getDefiningOp(), OpCode::shl
...
ComputeOperationNode *Graph::insertAddNode(Value result, DataType type) {
  ...
  this->op_list.push_back(std::make_unique<ComputeOperationNode>(
      NodeInfo(op_list.size(), name), OperationNode::OperationType::BinaryType,
      type, result.getDefiningOp(), OpCode::add
...
ComputeOperationNode *Graph::insertSubNode(Value result, DataType type) {
  ...
  this->op_list.push_back(std::make_unique<ComputeOperationNode>(
      NodeInfo(op_list.size(), name), OperationNode::OperationType::BinaryType,
      type, result.getDefiningOp(), OpCode::sub
...
ComputeOperationNode *Graph::insertAndiNode(Value result, DataType type) {
  ...
  this->op_list.push_back(std::make_unique<ComputeOperationNode>(
      NodeInfo(op_list.size(), name), OperationNode::OperationType::BinaryType,
      type, result.getDefiningOp(), OpCode::andi
...
ComputeOperationNode *Graph::insertOriNode(Value result, DataType type) {
  ...
  this->op_list.push_back(std::make_unique<ComputeOperationNode>(
      NodeInfo(op_list.size(), name), OperationNode::OperationType::BinaryType,
      type, result.getDefiningOp(), OpCode::ori
...
ComputeOperationNode *Graph::insertDivsiNode(Value result, DataType type) {
  ...
  this->op_list.push_back(std::make_unique<ComputeOperationNode>(
      NodeInfo(op_list.size(), name), OperationNode::OperationType::BinaryType,
      type, result.getDefiningOp(), OpCode::div
...
ComputeOperationNode *Graph::insertMulNode(Value result, DataType type) {
  ...
  this->op_list.push_back(std::make_unique<ComputeOperationNode>(
      NodeInfo(op_list.size(), name), OperationNode::OperationType::BinaryType,
      type, result.getDefiningOp(), OpCode::mul
...
ReductionNode *Graph::insertReductionNode(Value result, DataType type) {
  ...
  this->op_list.push_back(std::make_unique<ReductionNode>(
      NodeInfo(op_list.size(), name),
      OperationNode::OperationType::ReductionType, type, result.getDefiningOp(),
      code));
...
BitCastNode *Graph::insertBitCastNode(arith::IndexCastOp op) {
  ...
  this->op_list.push_back(std::make_unique<BitCastNode>(
      NodeInfo(op_list.size(), name), op.getOperation()));

  auto ff =
      std::find_if(op_list.begin(), op_list.end(), [&op](auto &arg) -> bool {
        return static_cast<BitCastNode *>(arg.get())->getParentOp() ==
               op.getOperation();
      });
...
MergeNode *Graph::insertMergeNode(dataflow::MergeOp op) {
  ...
  this->op_list.push_back(std::make_unique<MergeNode>(
      NodeInfo(op_list.size(), name), op.getOperation(), op->getParentOp()));

  auto ff =
      std::find_if(op_list.begin(), op_list.end(), [&op](auto &arg) -> bool {
        return static_cast<MergeNode *>(arg.get())->getParentOp() ==
               op.getOperation();
      });
...
ExecutionBlockNode *
Graph::insertExecutionBlockNode(dataflow::ExecutionBlockOp op) {
  ...
  this->exe_block_list.push_back(std::make_unique<ExecutionBlockNode>(
      NodeInfo(exe_block_list.size(), name), op, mask));
  auto ff = std::find_if(
      this->exe_block_list.begin(), this->exe_block_list.end(),
      [op](auto &arg) -> bool {
        return static_cast<ExecutionBlockNode *>(arg.get())->getRelatedOp() ==
               op;
      });
...
StateBranchNode *Graph::insertStateBranchNode(dataflow::StateOp op) {
  ...
  this->op_list.push_back(std::make_unique<StateBranchNode>(
      NodeInfo(this->op_list.size(), name), op));
  auto ff = std::find_if(
      this->op_list.begin(), this->op_list.end(), [op](auto &arg) -> bool {
...
AddressGenNode *Graph::insertAddressGenNode(dataflow::AddressOp op) {
  ...
  this->op_list.push_back(std::make_unique<AddressGenNode>(
      NodeInfo(this->op_list.size(), name), op));
  auto ff = std::find_if(
      this->op_list.begin(), this->op_list.end(), [op](auto &arg) -> bool {
...
SelectNode *Graph::insertSelectNode(dataflow::SelectOp op) {
  ...
  this->op_list.push_back(std::make_unique<SelectNode>(
      NodeInfo(this->op_list.size(), name), op, dataSelect));
  auto ff = std::find_if(
      this->op_list.begin(), this->op_list.end(), [op](auto &arg) -> bool {
...
LSNode *Graph::insertLoadNode(Value result, DataType type) {
  ...
  auto ff = std::find_if(
      this->op_list.begin(), this->op_list.end(), [&result](auto &arg) -> bool {
        return arg.get()->getOperation() == result.getDefiningOp();
      });
  return static_cast<LSNode *>(ff->get());
}

LSNode *Graph::insertStoreNode(Value result, DataType type,
                               mlir::Operation *op) {
  ...
  auto ff = std::find_if(
      ...
      [&op](auto &arg) -> bool { return arg.get()->getOperation() == op; });
  return static_cast<LSNode *>(ff->get());
}

AllocaNode *Graph::insertAllocaNode(Value result,
                                    OperationNode::OperationType optype,
                                    uint32_t size, uint32_t num_byte) {
  std::string name = "alloca_" + std::to_string(this->op_list.size());
  this->op_list.push_back(std::make_unique<AllocaNode>(
      NodeInfo(this->op_list.size(), name),
      OperationNode::OperationType::AllocaType, DataType::MemrefType,
      result.getDefiningOp(), size, num_byte));
  auto ff = std::find_if(
      this->op_list.begin(), this->op_list.end(), [&result](auto &arg) -> bool {
        return arg.get()->getOperation() == result.getDefiningOp();
      });
...
MemoryNode *Graph::createBufferMemory(AllocaNode *alloca, uint32_t size,
                                      uint32_t num_byte) {
...
MemoryNode *Graph::createBufferMemory(uint32_t id, uint32_t size,
                                      uint32_t num_byte) {
  ...
  auto ff = std::find_if(
      this->scratchpad_memories.begin(), this->scratchpad_memories.end(),
      [&id](auto &arg) -> bool { return arg.get()->getMemID() == id; });
...
ReturnNode *Graph::insertReturnNode(mlir::func::ReturnOp returnop) {
  ...
  this->op_list.push_back(std::make_unique<ReturnNode>(
      NodeInfo(this->op_list.size(), name),
      OperationNode::OperationType::ReturnType, returnop.getOperation()));
  auto ff = std::find_if(this->op_list.begin(), this->op_list.end(),
                         [&returnop](auto &arg) -> bool {
                           return arg.get()->getOperation() ==
                                  returnop.getOperation();
                         });
...
void Graph::dumpECDFG() {
  ...
  //   _out_file << "The number of Node: "  <<
  //   op_list.size()+arg_list.size()+loop_nodes.size()<< "\n"; _out_file <<
  //   "The number of Edge: "  << edge_list.size() << "\n"; _out_file << "The
  //   number of Block: "  << exe_block_list.size() << "\n"; _out_file.close();
}
