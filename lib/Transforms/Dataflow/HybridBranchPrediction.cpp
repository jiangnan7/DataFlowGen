  ...
  LogicalResult matchAndRewrite(dataflow::IfOp ifop,
    ...
    if (hasElseRegion) {
      ...
      if (ifop.getElseRegion().front().getOperations().size() == 1) {
        ...
        // for(int i=0; i <
        // ifop.getThenRegion().front().getTerminator().getNumOperands(); i++){

        // Handles the case of having more than one carry variable and yield
        // value.
        auto ifSel = rewriter.create<dataflow::SelectOp>(
            rewriter.getUnknownLoc(), ifop.getResults().getType(),
            condop->getResult(0), // lowerBound, lowerBound);
  ...
  void runOnOperation() override {
    ...
    func->walk([&](dataflow::IfOp ifop) {
      ...
      if (!ifop.getElseRegion().empty()) {
        ...
        //   auto value =
        //   ifop.getThenRegion().front().getTerminator()->getOperand(i); auto
        //   data_select =
        //   builder.create<dataflow::SelectOp>(builder.getUnknownLoc(),
        //   value.getType(), ifop.getCondition(), value,
        //   ifop.getElseRegion().front().getTerminator()->getOperand(i));
        //   data_select->setAttr("Select",
        //   StringAttr::get(builder.getContext(), "Data"));
        // }
        llvm::SmallVector<Value, 8> yieldValue;
        for (const auto &result : ifop.getResults()) {
          auto data_select = builder.create<dataflow::SelectOp>(
              builder.getUnknownLoc(), result.getType(), ifop.getCondition(),
              result, result);
          data_select->setAttr("Select",
                               StringAttr::get(builder.getContext(), "Data"));
          data_select->setAttr(
              "Data", StringAttr::get(builder.getContext(), "IF-THEN-ELSE"));
          yieldValue.push_back(data_select.getResult());
        }
        ifop.getOperation()->getBlock()->getTerminator()->setOperands(
            ValueRange{yieldValue});
      } else {
        //?
        int i = 3;
        for (const auto &result : ifop.getResults()) {
          auto data_select = builder.create<dataflow::SelectOp>(
              builder.getUnknownLoc(), result.getType(), ifop.getCondition(),
              result, ifop->getParentOp()->getOperand(i++));
          data_select->setAttr("Select",
                               StringAttr::get(builder.getContext(), "Data"));
        }
        ...
        builder.create<dataflow::StateOp>(builder.getUnknownLoc(),
                                          ifop.getCondition(), if_then,
                                          if_else_null);
      }
