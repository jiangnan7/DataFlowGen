...
public:
  ...
  LogicalResult matchAndRewrite(AffineForOp op,
    ...
    //   lowerBound = rewriter.create<arith::ConstantIntOp>(loc,
    //   op.getConstantLowerBound(), 32); upperBound =
    //   rewriter.create<arith::ConstantIntOp>(loc, op.getConstantUpperBound(),
    //   32); step = rewriter.create<arith::ConstantIntOp>(loc, op.getStep(),
    //   32);
    // } else {
    ...
    for (auto attr : op->getAttrs()) {
      if (attr.getName().str() == "Loop_Band" ||
          attr.getName().str() == "Loop_Level") {
...
public:
  ...
  matchAndRewrite(scf::IfOp op, OpAdaptor adaptor,
    ...
    auto newOp = rewriter.create<dataflow::IfOp>(
        op.getLoc(), resultTypes, op.getCondition(), hasElseRegion);
...
public:
  ...
  LogicalResult matchAndRewrite(AffineLoadOp op,
    ...
    auto newLoadOp = rewriter.create<memref::LoadOp>(
        op.getLoc(), op.getMemRef(), *resultOperands);
...
public:
  ...
  LogicalResult matchAndRewrite(arith::RemSIOp remOp,
                                PatternRewriter &rewriter) const override {
...
//   }
// };
struct OptimizeDataflow : public OptimizeDataflowBase<OptimizeDataflow> {
