...
std::vector<unsigned> affineExprAnalysis(
  ...
  if (auto binExpr = affineExpr.dyn_cast<mlir::AffineBinaryOpExpr>()) {
    ...
    case AffineExprKind::Mod:
      /// RHS of mod is always a constant or a symbolic expression with a
      /// positive value.
      break;
...
struct AnalyzeMemrefAddress : AnalyzeMemrefAddressBase<AnalyzeMemrefAddress> {
  ...
  void runOnOperation() {
    ...
    llvm::errs()
        << "---------analyzing the loop levels of the kernel--------\n";
          ...
          visitOp = [&](mlir::Operation *op, int currentDepth) -> int {
            ...
            if (auto forOp = dyn_cast<AffineForOp>(op)) {
              ...
              for (auto &innerOp : forOp.getBody()->getOperations()) {
                innerMax =
                    std::max(innerMax, visitOp(&innerOp, currentDepth + 1));
              }
              return innerMax;
            } else {
              return currentDepth;
    ...
    for (auto &elem : LoopLevels) {
      ...
      while (!toTraverse.empty()) {
        ...
        for (auto &op : currentFor.getBody()->getOperations()) {
          if (auto innerFor = dyn_cast<AffineForOp>(op)) {
          ...
          } else if (dyn_cast<AffineLoadOp>(op) ||
                     dyn_cast<AffineStoreOp>(op)) {
            ...
            for (unsigned dim = memoryShape.size() - 1; dim > 0; dim--) {
              subDimsSpaceMap[dim - 1] =
                  subDimsSpaceMap[dim] * memoryShape[dim];
            }
            ...
            // Traverse all the affineExpr, and extract coeff of each loop
            // level. Finally multiply with memory size in this dimension
            for (auto affineExpr : LSAffineMap.getResults()) {
              std::pair<std::map<unsigned, int64_t>, int64_t> factorTable;
              factorTable.second = 0;
          ...
          } else if (dyn_cast<memref::LoadOp>(op) ||
                     dyn_cast<memref::StoreOp>(op)) {
            llvm::errs() << "--------------------------------------------------"
                            "-----------\n";
            ...
            if (auto load = dyn_cast<memref::LoadOp>(op)) {
              ...
              // addrValues.insert(addrValues.end(), load.getIndices().begin(),
              // load.getIndices().end());
            } else if (auto store = dyn_cast<memref::StoreOp>(op)) {
              indices = store.getIndices();
              // addrValues.insert(addrValues.end(), store.getIndices().begin(),
