...
LaunchOp heteacc::launchBlock(Block *block) {
  ...
  LaunchOps.splice(launchBlock.begin(), parentOps, std::next(parentOps.begin()),
                   std::prev(parentOps.end()));
...
ExecutionBlockOp heteacc::executionBlock(Block *block) {
  ...
  if (!block->getOps<ExecutionBlockOp>().empty() ||
      !isa<dataflow::ForOp, mlir::AffineForOp, dataflow::IfOp>(
          block->getParentOp()))
  ...
  auto &parentOps = block->getOperations();
  exeOps.splice(exeBlock.begin(), parentOps, std::next(parentOps.begin()),
                std::prev(parentOps.end()));
...
void heteacc::applyBranchPrediction(dataflow::ExecutionBlockOp exeop) {
  ...
  for (auto &op : exeop.getBody().front()) {
    ...
    if (isa<dataflow::IfOp>(op)) {
      ...
      bool hasElseRegion =
          !dyn_cast<dataflow::IfOp>(op).getElseRegion().empty();
      ...
      order2block[branchStauts] =
          &dyn_cast<dataflow::IfOp>(op).getThenRegion().front();
      branch2block[&op].insert(
          &dyn_cast<dataflow::IfOp>(op).getThenRegion().front());
      // TODO: Using a better way to capture branch states and blocks.
      dyn_cast<dataflow::IfOp>(op).getElseRegion().front().walk(
          [&](dataflow::IfOp ifop) {
            branchStauts += 1;

            branchOps.push_back(ifop.getOperation());
            branch2block[ifop.getOperation()].insert(
  ...
  builder.create<dataflow::StateOp>(
      builder.getUnknownLoc(), dyn_cast<dataflow::IfOp>(topOp).getCondition(),
      if_then, if_else);
  ...
  for (auto &b2b : branch2block) {
    ...
    if (b2b.second.size() > 1) {
      builder.create<dataflow::StateOp>(
          ...
          dyn_cast<dataflow::IfOp>(b2b.first).getCondition(), if_then, if_else);
    } else {
      builder.create<dataflow::StateOp>(
          builder.getUnknownLoc(),
          dyn_cast<dataflow::IfOp>(b2b.first).getCondition(), if_then,
          if_else_null);
    }
  }
}
