...
struct LoopInfo {
  ...
  // Loop -> Loop
  llvm::DenseMap<Value, llvm::SmallSetVector<dataflow::ForOp, 8>>
      live_in_out_loop;
  ...
  // Live-out
  // Loop -> ins (output)
  llvm::DenseMap<Value, llvm::SmallSetVector<Operation *, 8>> live_out_out_ins;
  ...
  llvm::DenseMap<Value, llvm::SmallVector<mlir::Operation *, 8>>
      carry_dependencies;

  llvm::DenseMap<Value, llvm::SmallVector<mlir::Operation *, 8>>
      carry_dependencies_without_connection;
...
class GraphGen : public GraphVisitorBase<GraphGen, void>, public Graph {
...
public:
  ...
  llvm::DenseMap<mlir::Operation *, llvm::SmallSetVector<Node *, 8>>
      op_built_node;
  ...
  llvm::DenseMap<mlir::Value, llvm::SmallSetVector<dataflow::ForOp, 8>>
      live_in_ins_loop_edge;

  llvm::DenseMap<mlir::Value,
                 llvm::DenseSet<std::pair<dataflow::ForOp, dataflow::ForOp>>>
      live_in_loop_loop_edge;

  llvm::DenseMap<dataflow::ForOp,
                 llvm::DenseSet<std::pair<mlir::Value, mlir::Operation *>>>
      live_in_loop_ins_edge;

  llvm::DenseMap<std::pair<Value, mlir::Operation *>, ArgumentNode *>
      loop_edge_map;
  llvm::DenseMap<std::pair<Value, mlir::Operation *>, int>
      edge_direction_map; // 1: input, 2: output, 3. in/out
  ...
  llvm::DenseMap<mlir::Value,
                 llvm::SmallVector<std::pair<dataflow::ForOp, dataflow::ForOp>>>
      loop_loop_edge_lout_map;

  llvm::DenseMap<dataflow::ForOp,
                 llvm::DenseSet<std::pair<Value, mlir::Operation *>>>
  ...
  GraphGen(NodeInfo _n_info, llvm::raw_ostream &out)
      : dependency_graph(std::make_unique<Graph>(_n_info, out)), code_out(out) {
  }
  // Visit
public:
  ...
  void visitOp(dataflow::ExecutionBlockOp op) {
    this->map_op_node[op.getOperation()] =
        this->dependency_graph->insertExecutionBlockNode(op);
  }

  void visitOp(dataflow::StateOp op) {
    this->map_op_node[op.getOperation()] =
        this->dependency_graph->insertStateBranchNode(op);
  }
  ...
  void visitOp(vector::ReductionOp op) {
    ...
    auto reduction_op = this->dependency_graph->insertReductionNode(
        op.getResult(), DataType::VectorType);
    unsigned laneSize =
        getVectorLaneSize(dyn_cast<mlir::VectorType>(valueType));
