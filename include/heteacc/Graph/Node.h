...
public:
  ...
  void setName(const std::string &name) { this->info.Name = name; }
  void setID(uint32_t id) { this->info.ID = id; }
  NodeType getType() const { return this->node_type; }

  uint32_t getLaneNums() const { return this->lane_num; }

  void setLaneNums(const uint32_t nums) { this->lane_num = nums; }
  ...
  virtual std::string printInputEnable(PrintType, uint32_t) {
    return this->info.Name +
           std::string(" Input EnableInput with ID Not defined!");
  }
  virtual std::string printInputEnable(PrintType, std::pair<Node *, PortID>) {
    return this->info.Name +
           std::string(" Input1 EnableInput with ID Not defined!");
  }
  ...
  // input
  auto inputDataport_begin() { return this->port_data.data_input_port.begin(); }
  ...
  // mem
  auto read_req_begin() { return this->read_port_data.memory_req_port.begin(); }
  ...
  auto write_resp_end() { return this->write_port_data.memory_resp_port.end(); }
  ...
  bool checkOutputConfict(uint32_t _port_id) {
    return std::find(conflict_output_index.begin(), conflict_output_index.end(),
                     _port_id) != conflict_output_index.end();
  }
  bool checkInputConfict(uint32_t _port_id) {
    return std::find(conflict_input_index.begin(), conflict_input_index.end(),
                     _port_id) != conflict_input_index.end();
  }
};
...
public:
  explicit OperationNode(NodeInfo _ni, OperationType _optype, DataType datatype,
                         mlir::Operation *operation, OpCode opcode)
      : Node(Node::ComputeNodeTy, _ni), op_type(_optype), data_type(datatype),
        parent_op(operation), op_code(opcode) {}

  explicit OperationNode(NodeInfo _ni, OperationType _optype,
                         mlir::Operation *operation)
      : Node(Node::ComputeNodeTy, _ni), op_type(_optype), parent_op(operation),
        op_code(OpCode::other) {}

  explicit OperationNode(NodeInfo _ni, OperationType _optype, DataType datatype,
                         mlir::Operation *operation)
      : Node(Node::ComputeNodeTy, _ni), op_type(_optype), data_type(datatype),
        parent_op(operation), op_code(OpCode::other) {}

  mlir::Operation *getOperation();

  void setStaticFlag(bool flag) { this->static_flag = flag; }
  bool getStaticFlag() { return this->static_flag; }
  DataType getDataType() const { return this->data_type; }
  OperationType getOperationType() const { return this->op_type; }
  OpCode geOpCodeEnum() const { return this->op_code; }
  std::string getOpcodeName() { return opCodeToString(this->geOpCodeEnum()); }
  mlir::Operation *getOp() const { return this->parent_op; }
  bool isMemrefType() const { return this->data_type == DataType::MemrefType; }
  bool isIntegerType() const {
    return this->data_type == DataType::IntegerType;
  }
  bool isFloatType() const { return this->data_type == DataType::FloatType; }

  static bool classof(const Node *T) {
    return T->getType() == NodeType::OperationNodeTy;
  }

  void setParentNode(ExecutionBlockNode *node) { this->parentNode = node; }
...
public:
  ConstNode(NodeInfo _ni, mlir::Operation *op, bool isint, DataType type,
    ...
    if (dataType == DataType::VectorType) {
      ...
      auto denseAttr = dyn_cast<mlir::DenseElementsAttr>(
          dyn_cast<arith::ConstantOp>(op).getValue());
      Type elementType =
          dyn_cast<mlir::VectorType>(denseAttr.getType()).getElementType();
  ...
  int getConstID() { return this->id; }
  static bool classof(const Node *T) { return T->getType() == Node::ConstTy; }

  int getValue() { return value; }

  mlir::Operation *getConstantOp() { return this->parent_op; }
  virtual std::string printDefinition(PrintType) override;
...
public:
  explicit ExecutionBlockNode(NodeInfo nf, dataflow::ExecutionBlockOp op,
                              bool ismask)
      : Node(Node::ExecutionBlockNodeTy, nf), relatedOp(op), mask(ismask) {}

  dataflow::ExecutionBlockOp getRelatedOp() { return relatedOp; }
  void setControlOp(mlir::Operation *op) { control_op = op; }
  bool getNodeType() { return this->mask; }
  uint32_t getNumMerge() const { return this->merge_list.size(); }
  ...
  auto operation_node_end() const { return this->operation_node_list.end(); }
  void setNodeType(bool _t) { this->mask = _t; }
  auto merge_begin() const { return this->merge_list.begin(); }
  auto merge_end() const { return this->merge_list.end(); }
  virtual std::string printDefinition(PrintType) override;
...
public:
  MergeNode(NodeInfo _ni, mlir::Operation *op = nullptr,
            mlir::Operation *block = nullptr)
      : OperationNode(_ni, OperationType::MergeType, op), parent_op(op),
        block_op(block) {}
  ...
  mlir::Operation *getParentOp() { return this->parent_op; }
  static bool classof(const OperationNode *T) {
    return T->getOperationType() == OperationType::MergeType;
  }

  void setParentNode(Node *_parent) { this->mask_block = _parent; }
  Node *getParentNode() { return this->mask_block; }
...
public:
  explicit ArgumentNode(NodeInfo ni, ArgumentType arg_type, DataType d_type,
  ...
  void setParentNode(Node *node) { this->parentNode = node; }
  ...
  ArgumentType getArgType() { return this->argType; }
  DataType getDataArgType() { return this->dataType; }
...
public:
  ...
  uint32_t getContainerType() const { return this->con_type; }

  ArgumentNode *insertLiveInArgument(Value Value,
  ...
  uint32_t numLiveInArgList(ArgumentNode::ArgumentType type, DataType dtype);
  uint32_t numLiveOutArgList(ArgumentNode::ArgumentType type);
  uint32_t numCarryDepenArgList(ArgumentNode::ArgumentType type);

  auto live_in_sets_begin() { return this->live_in_sets.begin(); }
  auto live_in_sets_end() { return this->live_in_sets.end(); }
  auto live_out_sets_begin() { return this->live_out_sets.begin(); }
  auto live_out_sets_end() { return this->live_out_sets.end(); }
  auto live_in_mems_begin() { return this->live_in_mems.begin(); }
  auto live_in_mems_end() { return this->live_in_mems.end(); }

  auto live_in_vals_begin() { return this->live_in_vals.begin(); }
  auto live_in_vals_end() { return this->live_in_vals.end(); }
  auto carry_depen_sets_begin() { return this->carry_depen_sets.begin(); }
  auto carry_depen_sets_end() { return this->carry_depen_sets.end(); }
  RegisterList getLiveInSets() { return this->live_in_sets; }
  ...
  RegisterList getCarryDepenSets() { return this->carry_depen_sets; }
};
...
public:
  explicit FineArgCallNode(NodeInfo _nf)
      : ContainerNode(_nf, ContainerNode::ContainType::FuncCallTy) {}
...
public:
  explicit ComputeOperationNode(NodeInfo _ni, OperationType optype,
                                DataType datatype, mlir::Operation *operation,
                                OpCode opcode)
      : OperationNode(_ni, OperationType::BinaryType, datatype, operation,
                      opcode) {}
...
public:
  explicit CmpNode(NodeInfo _ni, OperationType optype, DataType datatype,
                   mlir::Operation *operation, OpCode opcode)
      : OperationNode(_ni, OperationType::CmpType, datatype, operation,
                      opcode) {}
...
public:
  explicit ReductionNode(NodeInfo _ni, OperationType optype, DataType datatype,
                         mlir::Operation *operation, OpCode opcode)
      : OperationNode(_ni, OperationType::CmpType, datatype, operation,
                      opcode) {}
...
public:
  AllocaNode(NodeInfo _ni, OperationType type, mlir::Operation *operation)
      : OperationNode(_ni, type, operation), size(1), num_byte(0) {}

  AllocaNode(NodeInfo _ni, OperationType type, DataType data_type,
             mlir::Operation *operation, uint32_t _size, uint32_t _num_byte = 1)
      : OperationNode(_ni, type, data_type, operation), size(_size),
        num_byte(_num_byte) {}

  uint32_t getNumByte() { return this->num_byte; }
  void setNumByte(uint32_t _n) { this->num_byte = _n; }

  uint32_t getSize() { return this->size; }
  void setSize(uint32_t _n) { this->size = _n; }
...
public:
  enum memType { memory = 0, scratchpad };
...
public:
  ...
  uint32_t getMemSize() { return this->size; }
  uint32_t getMemID() { return this->mem_id; }
  void setMemSize(int64_t num) { this->size = num; }
  uint32_t getMemByte() { return this->num_byte; }

  memType getUnitType() { return this->mem_type; }

  void setType(memType type) { this->mem_type = type; }
...
public:
  ...
  void setIndeuctionVar(OperationNode *var) { this->induction_variable = var; }
  auto getInductionVar() { return this->induction_variable; }
...
public:
  ...
  void setMemoryUnit(MemoryNode *_node) { this->mem_node = _node; }
  MemoryNode *getMemoryUnit() { return this->mem_node; }
  void setRouteID(uint32_t _id) { this->route_id = _id; }
  bool isMemStore() { return this->isStore; }
  auto getRouteID() { return this->route_id; }
...
public:
  ...
  //   : OperationNode(_ni, InstType::BranchInstructionTy, _ins),
  //   ending_loop(_loop) {}
  ...
  //   return isa<OperationNode>(T) && classof(cast<OperationNode>(T));
  // }
  dataflow::StateOp getRelatedOp() { return relatedOp; }
  dataflow::EnableOp getControlOp() { return controlOp; }
...
public:
  ...
  //   return mask_node;
  // }
  dataflow::SelectOp getRelatedOp() { return relatedOp; }
  arith::SelectOp getArithRelatedOp() { return relatedArithOp; }
  void setDataSelect(bool value) { this->dataSelect = value; }
  // static bool
